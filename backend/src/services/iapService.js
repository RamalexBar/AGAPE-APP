...
const buildProductMap = () => {
  ...
  Object.values(PLANES).forEach(plan => {
    ...
    if (plan.apple_quarterly_id)  map[plan.apple_quarterly_id]  = { tipo: 'subscription', plan: plan.id, dias: 90 };
    if (plan.google_quarterly_id) map[plan.google_quarterly_id] = { tipo: 'subscription', plan: plan.id, dias: 90 };
